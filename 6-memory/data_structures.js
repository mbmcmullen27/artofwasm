...
// allocate 64k block of memory with a 32-bit view
const memory = new WebAssembly.Memory({initial: 1});
const mem_i32 = new Uint32Array(memory.buffer);

const obj_base_addr = 0;    // the address of the first byte ofour data
const obj_count = 32;       // the number of structures
const obj_stride = 16;      // 16-byte stride

// structure attribue offsets
const x_offset = 0;
const y_offset = 4;
...
( async () => {
    ...
    for ( let i = 0; i < obj_count; i++ ) {
        ...
        if (c) console.log(`obj[${i_str}] x=${x} y=${y} r=${r} collision=${c}`.red.bold);
        else console.log(`obj[${i_str}] x=${x} y=${y} r=${r} collision=${c}`.green);
    }
})();